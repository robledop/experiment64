                                         ...
                                         const char* buffer)
{
    ...
    int out_len = 0;
    const int consumed = decode_domain_name_internal(base, base_len, offset,
                                 ...
                                 int* answers_length)
{
    ...
    for (int i = 0; i < ancount; i++)
    {
        ...
        if (record_offset >= buffer_len)
            break;
        struct q_name* q_name = decode_domain_name(buffer, buffer_len,
...
int parse_message(char* buffer, size_t buffer_len, struct dns_message* message_out)
{
    ...
    message_out->questions = parse_questions(buffer, buffer_len,
                                             ntohs(message_out->header.qdcount),
                                             &reply_questions_length);

    int reply_answers_length = 0;
    message_out->answers = parse_answers(buffer, buffer_len, reply_questions_length,
