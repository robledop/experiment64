...
static int send_with_retry(int sockfd, const struct sockaddr_in *dest, const void *packet, size_t packet_len)
{
    for (int attempt = 0; attempt < 10; attempt++) {
        const ssize_t sent = sendto(sockfd, packet, packet_len, 0, (const struct sockaddr *)dest, sizeof(*dest));
        if (sent >= 0)
            return 0;
        usleep(10000);
    }
...
int main(int argc, char **argv)
{
    ...
    for (int seq = 1; seq <= count; seq++) {
        ...
        while (now_ms() < deadline) {
            ...
            const ssize_t n =
                recvfrom(sockfd, recvbuf, sizeof(recvbuf), MSG_DONTWAIT, (struct sockaddr *)&src, &srclen);
            if (n > 0) {
