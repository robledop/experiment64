...
static size_t terminal_cell_index(const terminal_state_t *term, uint16_t row, uint16_t col)
{
    return (size_t)row * (size_t)term->cols + (size_t)col;
}

static uint8_t terminal_effective_fg(const terminal_state_t *term)
{
    return term->reverse ? term->current_bg : term->current_fg;
}

static uint8_t terminal_effective_bg(const terminal_state_t *term)
{
    return term->reverse ? term->current_fg : term->current_bg;
}

static void terminal_set_cell_locked(terminal_state_t *term, uint16_t row, uint16_t col, char ch)
{
    const size_t idx = terminal_cell_index(term, row, col);
    term->cells[idx] = ch;
    term->fg[idx] = terminal_effective_fg(term);
    term->bg[idx] = terminal_effective_bg(term);
}

static void terminal_fill_cells_locked(terminal_state_t *term, size_t start, size_t end)
{
    if (start >= end)
        return;

    const uint8_t fg = terminal_effective_fg(term);
    const uint8_t bg = terminal_effective_bg(term);
...
static void terminal_reset_attributes(terminal_state_t *term)
{
...
static void terminal_clear_locked(terminal_state_t *term)
{
    term->cursor_row = 0;
    term->cursor_col = 0;
    term->saved_row = 0;
...
static int terminal_allocate_grid(terminal_state_t *term, uint16_t cols, uint16_t rows)
{
    ...
    terminal_clear_locked(term);
    return 0;
}

static int terminal_resize_grid_locked(terminal_state_t *term, uint16_t new_cols, uint16_t new_rows)
{
    if (!term || new_cols == 0 || new_rows == 0)
        return -1;
...
static void terminal_scroll_up_locked(terminal_state_t *term)
{
    if (term->rows <= 1)
        return;
    ...
    terminal_fill_cells_locked(term, start, start + row_size);
}

static void terminal_newline_locked(terminal_state_t *term)
{
    if (term->cursor_row + 1 >= term->rows)
        terminal_scroll_up_locked(term);
    else
        term->cursor_row++;
}

static void terminal_put_char_locked(terminal_state_t *term, char ch)
{
    terminal_set_cell_locked(term, term->cursor_row, term->cursor_col, ch);
...
static void terminal_erase_line_locked(terminal_state_t *term, int mode)
{
    const size_t row_start = (size_t)term->cursor_row * term->cols;

    if (mode == 2) {
        terminal_fill_cells_locked(term, row_start, row_start + term->cols);
        return;
    }

    if (mode == 1) {
        terminal_fill_cells_locked(term, row_start, row_start + (size_t)term->cursor_col + 1);
        return;
    }

    terminal_fill_cells_locked(term, row_start + term->cursor_col, row_start + term->cols);
}

static void terminal_erase_display_locked(terminal_state_t *term, int mode)
{
    const size_t cursor = terminal_cell_index(term, term->cursor_row, term->cursor_col);
    const size_t total = (size_t)term->rows * (size_t)term->cols;

    if (mode == 2) {
        terminal_fill_cells_locked(term, 0, total);
        return;
    }

    if (mode == 1) {
        terminal_fill_cells_locked(term, 0, cursor + 1);
        return;
    }

    terminal_fill_cells_locked(term, cursor, total);
}
...
static void terminal_apply_sgr_code(terminal_state_t *term, int code)
{
    if (code < 0)
        code = 0;
...
static void terminal_apply_csi_locked(terminal_state_t *term, char final_char)
{
    int params[16] = {0};
    ...
    case 'J':
        terminal_erase_display_locked(term, csi_param_or_default(params, param_count, 0, 0));
        break;
    case 'K':
        terminal_erase_line_locked(term, csi_param_or_default(params, param_count, 0, 0));
        break;
    case 'X':
    {
        int n = csi_param_or_default(params, param_count, 0, 1);
        ...
        if (end_col > term->cols)
            end_col = term->cols;
        const size_t start = terminal_cell_index(term, term->cursor_row, term->cursor_col);
        const size_t end = terminal_cell_index(term, term->cursor_row, end_col);
        terminal_fill_cells_locked(term, start, end);
        break;
    }
    case 's':
    ...
    {
        if (param_count == 0)
            terminal_apply_sgr_code(term, 0);
        else {
            for (int i = 0; i < param_count; i++)
                terminal_apply_sgr_code(term, params[i]);
        }
        break;
    }
    default:
...
static void terminal_process_byte_locked(terminal_state_t *term, uint8_t byte)
{
    ...
    if (term->parser_state == TERM_PARSER_CSI) {
        if (byte >= '@' && byte <= '~') {
            terminal_apply_csi_locked(term, (char)byte);
            term->parser_state = TERM_PARSER_NORMAL;
            term->csi_len = 0;
            term->csi_buf[0] = '\0';
            return;
    ...
    if (byte >= 32 && byte <= 126)
        terminal_put_char_locked(term, (char)byte);
}
...
static size_t terminal_translate_key(terminal_state_t *term, const wm_event_key_t *ev, char *out, size_t out_cap)
{
    ...
    c = use_shift ? (unsigned char)scancode_to_char_shifted[code]
...
{
    terminal_state_t *term = (terminal_state_t *)arg;
    uint8_t buf[256];

    while (__atomic_load_n(&term->running, __ATOMIC_RELAXED)) {
        const ssize_t n = read(term->master_fd, buf, sizeof(buf));
        if (n <= 0)
            break;

        pthread_mutex_lock(&term->lock);
        for (ssize_t i = 0; i < n; i++)
            terminal_process_byte_locked(term, buf[i]);
        terminal_render_locked(term);
        pthread_mutex_unlock(&term->lock);
    }

    pthread_mutex_lock(&term->lock);
    term->shell_exited = true;
    terminal_render_locked(term);
    pthread_mutex_unlock(&term->lock);
    return nullptr;
}
...
int main(void)
{
    wm_window_t *win = wm_create_window(140, 70, DEFAULT_TERM_WIDTH, DEFAULT_TERM_HEIGHT, "Terminal");
    if (!win)
        return 1;

    video_context_t *context = context_new(win->buffer, win->width, win->height, (uint32_t)win->width * 4U);
    if (!context) {
        wm_destroy_window(win);
        return 2;
    }

    terminal_state_t term = {nullptr};
    term.win = win;
    ...
    const uint16_t initial_cols = terminal_cols_from_width(win->width);
    const uint16_t initial_rows = terminal_rows_from_height(win->height);
    if (terminal_allocate_grid(&term, initial_cols, initial_rows) != 0) {
        terminal_context_free(context);
        wm_destroy_window(win);
        return 3;
    }
    ...
    int slave_fd = -1;
    int shell_pid = -1;
    if (spawn_shell_on_pty(&master_fd, &slave_fd, &shell_pid, term.rows, term.cols, win->width, win->height) != 0) {
        free(term.cells);
        free(term.fg);
        free(term.bg);
        terminal_context_free(context);
    ...
    if (pthread_create(&reader_thread, nullptr, terminal_reader_thread, &term) == 0)
        reader_started = true;
    ...
    while (wm_next_event(event_buf, &event_type) == 0) {
        if (event_type == WM_EVENT_KEY) {
            const wm_event_key_t *ev = (const wm_event_key_t *)event_buf;
        ...
        if (event_type == WM_EVENT_WINDOW_RESIZED) {
            const wm_event_window_resized_t *ev = (const wm_event_window_resized_t *)event_buf;
            if (ev->window_id != win->window_id)
                continue;
            ...
            if (terminal_rebuild_context_locked(&term) == 0) {
                const uint16_t new_cols = terminal_cols_from_width(win->width);
                const uint16_t new_rows = terminal_rows_from_height(win->height);
                terminal_resize_grid_locked(&term, new_cols, new_rows);
                terminal_set_pty_winsize(term.master_fd, term.rows, term.cols, win->width, win->height);
                terminal_render_locked(&term);
            }
        ...
        if (event_type == WM_EVENT_WINDOW_CLOSED) {
            const wm_event_window_closed_t *ev = (const wm_event_window_closed_t *)event_buf;
            if (ev->window_id == win->window_id)
                break;
        }
    }
