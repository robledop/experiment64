...
- **Syscalls & features**: `execve` with argv/envp, `ioctl` (TTY window size, framebuffer queries, keyboard flush,
  network `GETNETINFO`; see `docs/ioctl.md`), `mmap`/`munmap` for `/dev/fb0`, `link`/`unlink`, `getcwd`, full `open`
  flag handling (create/trunc/append), `mmap`-backed framebuffer access, user pointer validation for canonical and
  user-mapped ranges
