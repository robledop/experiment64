...
static void *alloc_slab(const int index)
{
    ...
    list_foreach_entry(iter, &slab_caches[index], list) {
        ...
        {
            boot_message(ERROR,
                         "heap: slab obj_size mismatch cache=%d slab=%p obj=%zu expected=%zu",
                         index,
                         iter,
                         iter->obj_size,
                         expected_size);
    ...
    if (slot < layout.slot_min || slot >= layout.page_end) {
        boot_message(ERROR,
                     "heap: free_list pointer out of range (slot=%p start=%p end=%p)",
                     slot,
                     layout.page_start,
                     layout.page_end);
        return nullptr;
    ...
    if (layout.payload_size) {
        ...
        for (size_t i = 0; i < layout.payload_size; i++) {
            if (payload[i] != POISON_FREE) {
                boot_message(ERROR,
                             "heap: slot poison mismatch (cache=%d slab=%p slot=%p idx=%zu val=0x%x)",
                             index,
                             slab,
                             slot,
                             i + link_size,
                             payload[i]);
                panic("heap: slot poison mismatch");
                break;
            }
        }
