/**
 * @file scheduler.c
 * @brief Per-CPU, xv6-style round-robin scheduler.
 *
 * One scheduler pseudo-thread per CPU runs scheduler_loop(), which picks the
 * next runnable thread and context-switches into it; preempted threads return
 * here. The pieces a reader usually goes hunting for all live in this file:
 *
 *   - Policy:      find_any_runnable_thread_rr() / scheduler_find_next_thread_locked()
 *   - Preemption:  scheduler_tick() — runs every PIT tick from the timer ISR
 *                  (kernel/arch/x86_64/idt.c)
 *   - Switching:   scheduler_loop() and schedule() (paired with switch.S)
 *   - Reaping:     scheduler_claim_auto_reap_locked() + detached-thread cleanup
 *   - Bootstrap:   process_init(), create_scheduler_thread(), create_idle_thread()
 *
 * scheduler_lock serializes selection, validation and bookkeeping; it is NOT
 * held while a normal thread is actually running.
 */
...
/** Move a thread to the tail of its process thread list (round-robin fairness). */
...
/*
 * Scheduling policy and bookkeeping, all run under scheduler_lock: thread
 * selection (round-robin), the preemption tick, next-thread validation,
 * auto-reap claiming, and detached-thread cleanup.
 */
...
static void scheduler_collect_detached_terminated_threads(list_item_t *free_list)
{
    spinlock_assert_held(&scheduler_lock);
    process_t *process;
...
static process_t *scheduler_claim_auto_reap_locked(void)
{
    spinlock_assert_held(&scheduler_lock);
    process_t *process;
    list_foreach_entry(process, &process_list, list) {
...
static thread_t *scheduler_find_next_thread_locked(cpu_t *cpu)
{
    spinlock_assert_held(&scheduler_lock);
...
static bool scheduler_validate_next_thread_locked(thread_t *next, const char *ctx)
{
    spinlock_assert_held(&scheduler_lock);
    if (!next)
        return false;
...
static void scheduler_release_thread_list(list_item_t *free_list)
{
    thread_t *thread;
    thread_t *next;
