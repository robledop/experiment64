...
TEST(test_fb_ioctl_basic)
{
    ...
    const struct limine_framebuffer *fbhw = framebuffer_current();
...
TEST(test_fb_mmap_and_munmap)
{
    ...
    const struct limine_framebuffer *fb = framebuffer_current();
    TEST_ASSERT(fb != nullptr);
    const uint64_t fb_addr = (uint64_t)fb->address;
    const uint64_t fb_phys = (fb_addr >= g_hhdm_offset) ? (fb_addr - g_hhdm_offset) : fb_addr;
    ...
    const uint64_t map_base = (uint64_t)map & ~(PAGE_SIZE - 1);
    const uint64_t phys = vmm_virt_to_phys(kernel_process->pml4, map_base);
