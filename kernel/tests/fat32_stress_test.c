...
TEST(test_fat32_stress)
{
    ...
    printf("Phase 1: Creating and Writing %d files...\n", STRESS_FILE_COUNT);
    for (int i = 0; i < STRESS_FILE_COUNT; i++)
    {
        ...
        if (!node)
        {
            printf("Failed to resolve file %s\n", filename);
            return false;
        }
        ...
        {
            printf("Failed to write all data to %s (wrote %llu)\n", filename, written);
            vfs_close(node);
            return false;
        }
    ...
    printf("Phase 2: Reading and Verifying...\n");
    for (int i = 0; i < STRESS_FILE_COUNT; i++)
    {
        char filename[64];
        snprintf(filename, sizeof(filename), "%s/FILE%d.TXT", dirname, i);
        ...
        {
            printf("Failed to read all data from %s (read %llu)\n", filename, read);
            vfs_close(node);
            return false;
        }
        ...
        {
            printf("Data verification failed for %s\n", filename);
            vfs_close(node);
            return false;
        }
