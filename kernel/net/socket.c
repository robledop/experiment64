...
{
    for (;;) {
        socket_rx_packet_t* pkt = nullptr;
        WITH_LOCK(sock->rx_lock) {
            if (list_empty(&sock->rx_queue))
                return;
            pkt = list_entry(sock->rx_queue.next, socket_rx_packet_t, list);
            list_del(&pkt->list);
...
static void socket_accept_purge(socket_t* sock)
{
    ...
    for (;;)
    {
        ...
        WITH_LOCK(sock->accept_lock) {
            if (list_empty(&sock->accept_queue))
                return;
            child = list_entry(sock->accept_queue.next, socket_t, accept_list);
                       ...
                       const uint8_t src_ip[static 4], uint16_t src_port, const uint8_t* payload, size_t payload_len)
{
    WITH_LOCK(socket_lock) {
        ...
        list_foreach_entry(s, &socket_list, list)
        {
            ...
            from.port = src_port;
            return socket_enqueue_rx(s, payload, payload_len, &from);
        }
    }
    return -1;
}
