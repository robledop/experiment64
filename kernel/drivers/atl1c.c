...
static bool atl1c_enable_msi(const struct pci_device device, const uint8_t vector)
{
    ...
    uint8_t cap_ptr = (uint8_t)(pci_config_read_word(device.bus, device.slot, device.function,
    ...
    for (uint8_t i = 0; cap_ptr != 0 && i < 48; i++)
    {
        ...
        if (cap_id == 0x05)
        {
            const uint16_t msg_ctrl = pci_config_read_word(device.bus, device.slot, device.function,
                                                           (uint8_t)(cap_ptr + 2));
            ...
            pci_config_write_word(device.bus, device.slot, device.function, (uint8_t)(cap_ptr + 4),
                                  (uint16_t)(msg_addr & 0xFFFFu));
            pci_config_write_word(device.bus, device.slot, device.function, (uint8_t)(cap_ptr + 6),
            ...
            pci_config_write_word(device.bus, device.slot, device.function, (uint8_t)(cap_ptr + 2),
...
static void atl1c_reset_pcie(const struct pci_device device)
{
    ...
    if (status & PCI_STATUS_CAPABILITIES_LIST)
    {
        ...
        for (uint8_t i = 0; cap_ptr != 0 && i < 48; i++)
        {
            ...
            if (cap_id == 0x05)
            {
                const uint16_t ctrl = pci_config_read_word(device.bus, device.slot, device.function,
                                                           (uint8_t)(cap_ptr + 2));
                pci_config_write_word(device.bus, device.slot, device.function, (uint8_t)(cap_ptr + 2),
                                      (uint16_t)(ctrl & ~0x1u));
            }
