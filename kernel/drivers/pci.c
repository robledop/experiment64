...
#define PCI_HEADER_WORDS 32
#define PCI_FUNCTION_ZERO 0
...
static const struct pci_driver pci_drivers[] = {
    ...
    {.class     = PCI_CLASS_NETWORK,
     .subclass  = PCI_SUBCLASS_ETHERNET,
     .vendor_id = ATHEROS_VEND,
     .device_id = AR8162_DEV,
     .init      = &atl1c_init  },
};
 ...
 */
static const char *pci_find_name(const uint8_t class, const uint8_t subclass)
{
    for (size_t i = 0; i < ARRAY_SIZE(pci_classes); i++) {
        if (pci_classes[i].class == class && pci_classes[i].subclass == subclass) {
...
{
    for (size_t i = 0; i < ARRAY_SIZE(pci_vendors); i++) {
...
static USED void pci_dump_bars(const struct pci_header *pci)
{
    ...
    for (uint8_t i = 0; i < 6; i++) {
        ...
        logged = true;
        if (bar & PCI_BAR_IO) {
            boot_message(INFO, "  BAR%u: IO base=0x%08x raw=0x%08x", i, bar & ~0x3U, bar);
            continue;
        }
        ...
        boot_message(
            INFO, "  BAR%u: MEM32 base=0x%08x raw=0x%08x prefetch=%u", i, bar & ~0xFULL, bar, prefetch ? 1U : 0U);
    }
...
static USED void pci_dump_capabilities(const uint8_t bus, const uint8_t device, const uint8_t function,
                                       const struct pci_header *pci)
{
    ...
    for (uint8_t i = 0; cap_ptr != 0 && i < 48; i++) {
        ...
        boot_message(INFO, "    0x%02x %s (id=0x%02x)", cap_ptr, pci_cap_name(cap_id), cap_id);
...
static USED void pci_dump_device_config(const struct pci_header *pci, const uint8_t bus, const uint8_t device,
                                        const uint8_t function)
{
    boot_message(
        INFO, "PCI %02x:%02x.%u vendor=0x%04x device=0x%04x", bus, device, function, pci->vendor_id, pci->device_id);
    boot_message(INFO,
                 ...
                 pci->prog_if,
                 pci->revision_id);
    boot_message(INFO, "  command=0x%04x status=0x%04x header=0x%02x", pci->command, pci->status, pci->header_type);
    boot_message(INFO,
                 ...
                 pci->irq,
                 pci->interrupt_pin);
    boot_message(INFO, "  cache_line=%u latency=%u bist=0x%02x", pci->cache_line_size, pci->latency_timer, pci->BIST);
 ...
 */
static struct pci_device pci_device_from_header(const struct pci_header *pci, const uint8_t bus, const uint8_t device,
                                                const uint8_t function)
{
...
static void pci_log_device(const struct pci_device *dev)
{
    ...
    if (dev->class == PCI_CLASS_NETWORK && dev->subclass == PCI_SUBCLASS_ETHERNET) {
        boot_message(INFO,
    ...
    if (dev->class == PCI_CLASS_MULTIMEDIA && dev->subclass == PCI_SUBCLASS_AUDIO) {
        boot_message(INFO,
    ...
    if (dev->class == PCI_CLASS_SERIAL_BUS && dev->subclass == PCI_SUBCLASS_USB) {
        boot_message(
            INFO, "USB controller interface: %s (prog_if=0x%02X)", pci_usb_prog_if_name(dev->prog_if), dev->prog_if);
    }
}

static bool pci_driver_matches(const struct pci_driver *driver, const struct pci_device *dev)
{
    ...
    return class_match && subclass_match && vendor_match && device_match;
}

static void pci_bind_driver(const struct pci_header *pci, const uint8_t bus, const uint8_t device,
                            const uint8_t function)
{
    struct pci_device dev = pci_device_from_header(pci, bus, device, function);

    pci_log_device(&dev);

    for (size_t i = 0; i < ARRAY_SIZE(pci_drivers); i++) {
 ...
 */
static struct pci_header pci_read_header(const uint8_t bus, const uint8_t num, const uint8_t function)
{
    struct pci_header pci_data;
    for (uint8_t i = 0; i < PCI_HEADER_WORDS; i++) {
...
static void pci_scan_function(const uint8_t bus, const uint8_t slot, const uint8_t function)
{
    struct pci_header pci = pci_read_header(bus, slot, function);
    if (pci.vendor_id != PCI_INVALID_VENDOR_ID)
        pci_bind_driver(&pci, bus, slot, function);
}
...
void pci_scan()
{
    ...
    for (uint16_t bus = 0; bus < PCI_MAX_BUSES; bus++) {
        struct pci_header bus_header = pci_read_header((uint8_t)bus, 0, PCI_FUNCTION_ZERO);
        if (bus_header.vendor_id == PCI_INVALID_VENDOR_ID)
            continue;

        for (uint8_t slot = 0; slot < PCI_MAX_SLOTS; slot++) {
            struct pci_header slot_header = pci_read_header((uint8_t)bus, slot, PCI_FUNCTION_ZERO);
            if (slot_header.vendor_id == PCI_INVALID_VENDOR_ID)
                continue;

            pci_bind_driver(&slot_header, (uint8_t)bus, slot, PCI_FUNCTION_ZERO);

            for (uint8_t function = 1; function < PCI_MAX_FUNCTIONS; function++)
                pci_scan_function((uint8_t)bus, slot, function);
        }
    }
}
