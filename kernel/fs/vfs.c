/**
 * @file vfs.c
 * @brief Virtual filesystem: inode operations, mount table, path resolution,
 *        and boot-time root mount.
 *
 * This single file implements the kernel's VFS. It is organized top-to-bottom
 * in four layers — each depending only on what comes before it:
 *
 *   1. Core inode ops      — open/close/read/write/... dispatch via iops vtable
 *   2. Mount table         — register_mount / readdir / finddir with overlay
 *   3. Path resolution     — string paths → inode, plus mknod/link/unlink/rename
 *   4. Boot-time root mount — GPT scan, pick the boot device, attach subvolumes
 *
 * Reading the file straight through tells the whole VFS story.
 */
...
// =========================================================================
// Section 1: Core inode operations
//
// Every VFS call dispatches through the inode's iops vtable. An iop slot is
// allowed to be null; vfs_* wrappers fall back to sensible defaults when so.
// =========================================================================
...
// =========================================================================
// Section 2: Mount table
//
// A flat array of (name, inode) pairs with two special behaviours:
//   - vfs_check_mount() clones the mounted root so callers can freely release.
//   - vfs_readdir() on the root overlays virtual mounts (e.g. /dev) after the
//     on-disk entries, so `ls /` shows both.
// =========================================================================
...
// =========================================================================
// Section 3: Path resolution and mutating operations
//
// These walk a string path to an inode (vfs_resolve_path) or modify the
// namespace (mknod/link/unlink/rename). The `_hold` helpers avoid prematurely
// releasing an inode we still need as an anchor.
// =========================================================================
...
// =========================================================================
// Section 4: Boot-time root mount
//
// Scan all storage devices for GPT partitions, classify them by type GUID,
// and mount them:
//   - Root (Linux Filesystem)         → /
//   - Data  (Microsoft Basic Data)    → /mnt
//   - ESP   (EFI System Partition)    → /boot
//   - Extra root partitions           → /disk1, /usb
// Falls back to a FAT32 mount at LBA 2048 if no GPT root is found.
// =========================================================================
